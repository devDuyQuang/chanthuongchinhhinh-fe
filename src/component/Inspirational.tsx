  ...
  const items =
    data?.items && data.items.length > 0
      ? data.items.map((item, index) => ({
          ...
          svg:
            inspirationaldata[index % inspirationaldata.length]?.svg ||
            inspirationaldata[0].svg,
          columnstand:
            inspirationaldata[index % inspirationaldata.length]?.columnstand ||
            "",
          delay:
            inspirationaldata[index % inspirationaldata.length]?.delay ||
            "0.2s",
        }))
      ...
      <div className="container">
        <div className="row g-lg-5 align-items-center">
          <div className="col-lg-7 m-b10">
            ...
            {items.map((item, i) => (
              <div
                className={`icon-bx-wraper style-9 m-b20 ${item.columnstand} wow fadeInUp bg-light`}
